...
void Reset(void)
{
    ...
    for (int i = 0; i < 3; i++)
    {
    	AvgTime[i] = total_time[i]/(sentMessages[i]+blockedMessages[i]);
        printf("Sender %d - Sent: %d, Blocked: %d, AvgTime: %d\n", i, sentMessages[i],
        		blockedMessages[i], AvgTime[i]);
        AvgTimeTotal += AvgTime[i];
    }
    ...
    else
    {
        for (int i = 0; i < 3; i++)
         {
            TickType_t Start_period = rand() % (UpperBound[timerIndex] - LowerBound[timerIndex] + 1) + LowerBound[timerIndex];
            xTimerChangePeriod(senderTimers[i], pdMS_TO_TICKS(Start_period), 0);
         }
    }
}
...
int main(void)
{
	...
	if( messageQueue != NULL )
	{
				...
				&& sender_task[2] == pdPASS && receiver_task == pdPASS)
		{
			// Create timers
			for (int i = 0; i < 3; i++){
				TickType_t Start_period = rand() % (UpperBound[0] - LowerBound[0] + 1) + LowerBound[0];
				senderTimers[i] = xTimerCreate("SenderTimer", pdMS_TO_TICKS(Start_period), pdTRUE, (void *)i, SenderTimerCallback);
			}
